...
export const MobileNav = ({ navShow, setNavShow }: Props) => {
  ...
  return (
    <>
      <motion.button
        onClick={onToggleNav}
        className="flex items-center justify-center p-0 ml-1 text-lg bg-transparent h-11 w-11 text-typeface-primary dark:text-typeface-primary-dark sm:ml-4 sm:hidden"
        whileHover={{
          scale: 1.2,
      ...
      {isLoaded && navShow && (
        <motion.nav
          className="visible fixed left-0 right-0 top-[60px] bottom-0 z-50 block h-full w-full max-w-[100vw] overflow-hidden bg-body px-6 pb-6 dark:bg-body-dark sm:hidden"
          animate={{ x: 0 }}
          initial={{ x: '100vw' }}
          transition={{
            type: 'spring',
